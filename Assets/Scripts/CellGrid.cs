...
public class CellGrid : MonoBehaviour
{
    ...
    public List<Cell> emptyCells = new List<Cell>();//空cell List
    private int size;//cell数量
    private int height;//行高即行数
    private int witdh;//列长即列数
...
public Cell GetCell(Vector2Int coord)//根据坐标获取任一cell
    {
        ...
        if (coord.x > 0 && coord.x < witdh && coord.y > 0 && coord.y < height)
        {
...
public void FindEmptyCells()//统计board中所有空cell
    {
